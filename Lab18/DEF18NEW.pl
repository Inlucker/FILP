...
/*get_player(Player):-	pos(X, Y),
						Player = json{x:X, y:Y}.
get_blocks1(Blocks):-	findall(json{x:X, y:Y}, cell(X, Y, block1), Blocks).
...
move(Move):-reset_finish(), pos(X, Y), NX is X+1, cell(NX, Y, Cell), moved_to_cell(cell(NX, Y, Cell)),
			%pos(X2, Y2), format('Moved right, pos=[~w;~w]\n', [X2, Y2]),
			get_pole(Move).
move(Move):-reset_finish(), pos(X, Y), NY is Y+1, cell(X, NY, Cell), moved_to_cell(cell(X, NY, Cell)),
